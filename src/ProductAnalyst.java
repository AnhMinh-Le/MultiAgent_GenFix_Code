...
public class ProductAnalyst extends Chatter {
    ...
    public ProductAnalyst() {
        super("You are an assistant who helps extract package and class information from product descriptions.", 3, 10);
    }

    public void analyzeAndSaveToJson(String description, String outputPath) {
        // First, get package names
        String packagePrompt = "Analyze the following product description and list only the main package names that would be needed. Return the response as a comma-separated list:\n" + description;
        String packageResponse = chat(packagePrompt);
        String[] packages = packageResponse.split(",");

        JSONArray jsonArray = new JSONArray();

        // For each package, get its classes
        for (String pkg : packages) {
            pkg = pkg.trim();
            String classPrompt = "For the package '" + pkg + "', analyze the following description and list the classes needed with their descriptions. Format as 'ClassName: Description' with one class per line:\n" + description;
            String classResponse = chat(classPrompt);

            JSONObject packageObj = new JSONObject();
            packageObj.put("name", pkg);

            JSONArray classesArray = new JSONArray();
            String[] classes = classResponse.split("\n");
            for (String cls : classes) {
                if (cls.contains(":")) {
                    String[] parts = cls.split(":", 2);
                    JSONObject classObj = new JSONObject();
                    classObj.put("name", parts[0].trim());
                    classObj.put("descriptions", parts[1].trim());
                    classesArray.put(classObj);
                }
            }

            packageObj.put("classes", classesArray);
    ...
    """;
        String outputPath = "E:/Github Repo/OOP_MultiAgen_GenFix_Code/src/data/product.json";
        analyst.analyzeAndSaveToJson(description, outputPath);
        System.out.println("Analysis completed and saved to " + outputPath);
    }
}
